...
def get_plist_secrets(checksum, app_dir):
    ...
    logger.info(msg)
    append_scan_status(checksum, msg)
    # CHANGED: was set() — dicts not hashable, use list + seen
    result_list = []
    seen = set()
    ...
    for i in app_path.rglob('*.plist'):
        # CHANGED: compute relative path for UI linking
        rel_path = str(i.relative_to(app_path))
        ...
        for index, line in enumerate(xml_list):
            if '<key>' in line and is_secret_key(_remove_tags(line)):
                ...
                if value and ' ' not in value:
                    secret_str = f'{_remove_tags(line)} : {_remove_tags(value)}'
                    # CHANGED: dedup + store with path
